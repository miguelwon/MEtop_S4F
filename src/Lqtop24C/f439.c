...
REAL F439_out(double GG,REAL*DP,REAL*Q0,REAL*Q1,REAL*Q2)
{
/*
                           n2     !  n2                              
                        /==>======!==>==\                            
...
R=+DP[5]*(C[2]*(DP[0]*(DP[8]*(-DP[12]-DP[13])+DP[9]*(-DP[12]-DP[13])+DP[14]*
 ...
 DP[11]*(S[1]-DP[5]))-C[3]*DP[10]-C[6]*DP[11])+C[3]*(DP[14]*DP[0]-DP[11]*
 DP[6]))+S[1]*(DP[12]*(DP[7]*(S[1]*(4*DP[3]+8*DP[4])+C[4]*(-DP[1]-DP[4])-
 C[5]*DP[2]-C[2]*DP[3])+DP[0]*(DP[9]*(C[2]-4*S[1])+C[4]*DP[8])+DP[6]*(DP[4]*
 (4*S[1]-C[2])-C[4]*DP[2]))+DP[13]*(C[2]*(DP[8]*DP[0]-DP[6]*DP[2]-DP[7]*
 DP[1])-C[4]*DP[7]*DP[2]));
R*=(N/D)*Q2[1]*Q2[2];
 if(cb_coeff_out)
 {
