/*******************************
*    CalcHEP  3.2*
*******************************/
#include"num_out.h"
#include"num_in.h"
extern FNN F232_out;
static void C232(REAL * C)
{
REAL* V=va_out;
REAL S[8];                                                                  
     
S[0]=V[22]*V[22];
S[1]=V[8]*V[8];
C[5]=+S[1]*(V[19]*(24*V[22]+36*V[19])+4*S[0]);
C[4]=+V[19]*(24*V[22]+36*V[19])+4*S[0];
S[2]=V[8]*V[8]*V[8]*V[8];
C[3]=+S[2]*(V[19]*(6*V[22]+9*V[19])+S[0]);
C[2]=+S[1]*(V[19]*(12*V[22]+18*V[19])+2*S[0]);
S[3]=V[39]*V[39]*V[39]*V[39];
S[4]=V[14]*V[14]*V[14]*V[14];
S[5]=V[2]*V[2]*V[2]*V[2];
C[1]=+9*S[3]*S[4]*S[5];
S[6]=V[52]*V[52];
S[7]=V[1]*V[1]*V[1]*V[1];
C[0]=+S[6]*S[7];
}
REAL F232_out(double GG,REAL*DP,REAL*Q0,REAL*Q1,REAL*Q2)
{
/*
   U                       N2     !  N2                      U       
 ==<==\                 /==<======!==<==\                 /==<==     
   P1 |                 |  P3     !  P3 |                 |  P1      
      |                 |         !     |                 |          
   u  | ~0C    t     W+ |  e2     !  e2 |  W+    t    ~0C |  u       
 ==>==@-1---@==<==@-2<--@==>======!==>==@--<4-@==<==@---3-@==>==     
   P2    P7 |  P8 |  P9    P4     !  P4   -PC | -PB | -PA    P2      
            |     |               !           |     |                
            |     |        B      !  B        |     |                
            |     \========<======!==<========/     |                
            |              P6     !  P6             |                
            |                     !                 |                
            |              u      !  u              |                
            \==============>======!==>==============/                
                           P5     !  P5                              
*/
REAL N,D,R;
REAL * V=va_out;
static REAL C[6];REAL S[2];                                                 
     
if(CalcConst) C232(C);
N=-C[0];
S[0]=V[53]*V[53]*V[53]*V[53];
D=+C[1]*S[0];
S[1]=V[53]*V[53];
R=+DP[7]*(C[2]*(DP[5]*(DP[0]*(DP[12]*(DP[8]-DP[2]-DP[4]+DP[9])+DP[13]*(
 DP[8]-DP[2]-DP[4]+DP[9])+S[1]*(DP[14]-DP[11])+DP[5]*(DP[11]-DP[14]))+DP[6]*
 (DP[12]*(DP[2]+DP[4]-DP[8]-DP[9])+DP[13]*(DP[2]+DP[4]-DP[8]-DP[9])+S[1]*(
 DP[11]-DP[14])+DP[5]*(DP[14]-DP[11]))+DP[7]*(DP[1]*(-DP[12]-DP[13])+DP[3]*
 (-DP[12]-DP[13])+DP[10]*(DP[5]-S[1])))+S[1]*(DP[12]*(DP[0]*(DP[4]-DP[9])+
 DP[6]*(DP[9]-DP[4])+DP[7]*DP[3])+DP[13]*(DP[0]*(DP[2]-DP[8])+DP[6]*(DP[8]-
 DP[2])+DP[7]*DP[1])))+S[1]*(DP[12]*(S[1]*(C[4]*(DP[0]*(DP[9]-DP[4])+DP[6]*(
 DP[4]-DP[9])-DP[7]*DP[3]))+C[5]*(DP[0]*(DP[2]-DP[8])+DP[6]*(DP[8]-DP[2])+
 DP[7]*DP[1])))+DP[5]*(C[3]*(DP[0]*(DP[11]-DP[14])+DP[6]*(DP[14]-DP[11])+
 DP[10]*DP[7])));
R*=(N/D)*Q2[1]*Q2[2];
 if(cb_coeff_out)
 {
  cb_coeff_out[0] += (R*1)/(1);
 }
 return R;
}
